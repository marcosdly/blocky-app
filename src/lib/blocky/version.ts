...
export async function versionAsString(): Promise<string> {
  let stdout: string;
  try {
...
export async function versionAsStruct(): Promise<VersionStruct> {
  const versionString = await versionAsString();
